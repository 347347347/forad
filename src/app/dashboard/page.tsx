...
export default function DashboardPage() {
  ...
  async function search() {
    ...
    try {
      const res = await fetch(`/api/dashboard?title=${encodeURIComponent(query.trim())}`)
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <header className="bg-white border-b border-gray-200 px-6 py-4">
        <div className="max-w-6xl mx-auto flex items-center gap-4 flex-wrap">
          ...
          <div className="flex gap-2 flex-1 max-w-xl ml-auto">
            <input
              type="text"
              value={query}
              onChange={e => setQuery(e.target.value)}
              onKeyDown={e => e.key === 'Enter' && search()}
              placeholder="検証タイトルを入力（例：掃除機、ランニングシューズ）"
          ...
          <>
            ...
            <div className="flex items-center justify-between mb-4 flex-wrap gap-3">
              ...
              <div className="flex gap-2">
                ...
                <a
                  href={data.links.manual}
                  target="_blank"
                  rel="noopener noreferrer"
                  className="inline-flex items-center gap-1.5 h-8 px-3 text-xs font-medium border border-gray-200 rounded-lg text-gray-600 hover:bg-gray-50"
                >
                  <svg className="w-3 h-3" viewBox="0 0 12 12" fill="none">
                    <path d="M2 1.5h8v9H2z" stroke="currentColor" strokeWidth="1.2"/>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
              ...
              <div className="bg-white border border-gray-200 rounded-xl p-5">
                  ...
                  {axes.map(a => (
                    <div
                      ...
                      className={`flex items-start gap-2.5 p-2.5 rounded-lg border cursor-pointer transition-colors ${
                        a.done ? 'bg-gray-50 border-gray-200' : 'border-gray-200 hover:bg-gray-50'
                      }`}
                    >
                      <div className={`w-4 h-4 min-w-[16px] rounded border-[1.5px] flex items-center justify-center mt-0.5 transition-all ${
                        a.done ? 'bg-blue-600 border-blue-600' : 'border-gray-300'
                      }`}>
                        {a.done && (
                ...
                <div className="border-t border-gray-100 mt-4 pt-3 text-xs text-gray-400">出典: Notion</div>
              </div>
              ...
              <div className="flex flex-col gap-4">
                {/* Product list */}
                <div className="bg-white border border-gray-200 rounded-xl p-5">
                  <div className="text-xs font-medium text-gray-400 uppercase tracking-wide mb-3">商品リスト — import商品情報</div>
                  ...
                  <div className="border-t border-gray-100 mt-3 pt-3 text-xs text-gray-400">出典: Spreadsheet / import商品情報</div>
                </div>

                {/* Input items */}
                <div className="bg-white border border-gray-200 rounded-xl p-5">
                  <div className="text-xs font-medium text-gray-400 uppercase tracking-wide mb-3">検証項目（J列）— Spreadsheet</div>

                  <div className="text-xs font-medium text-gray-500 mb-2">【定量】検証結果入力フォーム</div>
                  <div className="flex flex-col divide-y divide-gray-50">
                    {data.quantInputs.map((item, i) => (
                      <div key={i} className="flex items-center justify-between py-1.5 text-sm">
                        <span className="text-gray-700">{item}</span>
                        <span className="text-xs bg-gray-100 text-gray-500 rounded px-1.5 py-0.5 font-medium">J列</span>
                      </div>
                    ))}
                  </div>

                  <div className="text-xs font-medium text-gray-500 mt-4 mb-2">【機能】検証結果入力フォーム</div>
                  <div className="flex flex-col divide-y divide-gray-50">
                    {data.funcInputs.map((item, i) => (
                      <div key={i} className="flex items-center justify-between py-1.5 text-sm">
                        <span className="text-gray-700">{item}</span>
                        <span className="text-xs bg-gray-100 text-gray-500 rounded px-1.5 py-0.5 font-medium">J列</span>
                      </div>
                  ...
                  <div className="border-t border-gray-100 mt-3 pt-3 text-xs text-gray-400">出典: Spreadsheet / J列</div>
                </div>
              </div>
            </div>
          </>
